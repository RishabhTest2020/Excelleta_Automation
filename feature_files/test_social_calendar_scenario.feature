Feature: Automation Test Plan Social Calendar Prod nad Test Envs BrowserStack

	Scenario Outline: Render All Templates of a Given Month
		Given I name a test verify social media calendar day: "<day>"
		Given I update headers through ModHeader
		Given User is logged in with shorter new auth calendar
		When I navigate to Social Calendar Page
		Then I render a video from calendar day "<locator>"
		And I check assertion for rendering all the templates
		Examples:
			| day  | locator |
			| 1st  | 1 |
			| 2nd  | 2 |
			| 3rd  | 3 |
			| 4th  | 4 |
			| 5th  | 5 |
			| 6th  | 6 |
			| 7th  | 7 |
			| 8th  | 8 |
			| 9th  | 9 |
			| 10th  | 10 |
			| 11th  | 11 |
			| 12th  | 12 |
			| 13th  | 13 |
			| 14th  | 14 |
			| 15th  | 15 |
			| 16th  | 16 |
			| 17th  | 17 |
			| 18th  | 18 |
			| 19th  | 19 |
			| 20th  | 20 |
			| 21st  | 21 |
			| 22nd  | 22 |
			| 23rd  | 23 |
			| 24th  | 24 |
			| 25th  | 25 |
			| 26th  | 26 |
			| 27th  | 27 |
			| 28th  | 28 |
			| 29th  | 29 |
			| 30th  | 30 |
			| 31th  | 31 |
